...
const RightContent1 = props => <Text style={styles.txt}>education</Text>
...
export default function EducationDetails({route}) {
  const navigation = useNavigation();
  const [education, setEducation] = React.useState([{}]);
  const getEducationHelp = async () => {
  ...
  return (
    <PaperProvider>
     <ScrollView style={styles.container}>
        ...
        <Text style={styles.txttop}>Education</Text>
        {education && education.map((educationItem, index) => (
        <Card style={styles.cardBox} key={index}>
          <Card.Title left={() => <LeftContent imageUrl={educationItem.imageUrl} />} />
          <Card.Content>
            <Text variant="titleLarge" style={styles.txt} key={index}>
              {educationItem.title} ____________________________
            </Text>
          </Card.Content>
          <Text variant="bodyMedium" style={styles.txtdetail} >Name: {educationItem.name} </Text>
          <Text variant="bodyMedium" style={styles.txtdetail2} >Location: {educationItem.location}</Text>
          <Text style={styles.details}>
            {educationItem.description}
          </Text> 
          <TouchableOpacity  
          onPress={() => navigation.navigate('Help', {Id: educationItem.id, type: 'Education'})}
